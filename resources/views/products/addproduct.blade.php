...
<body>
    ...
    <div class="container containerP">
        ...
        <form action="{{ url('/products/addproductbbdd') }}" method="POST" files='true' enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label for="id_category">Selecciona la categoría:</label>
                <select name="id_category" id="id_category" class="form-control input" name="id_category">
            ...
            <div class="form-group">
                ...
                <input type="nombre" class="form-control input @error('nombre') is-invalid @enderror" value="{{ old('nombre') }}" required id="nombre" name="nombre">
                @error('nombre')
            ...
            <div class="form-group">
                ...
                <textarea class="form-control input" id="description" name="description" rows="3" value="{{ old('description') }}" required></textarea>
            </div>

            <div class="form-group">
                <label for="precio">Introduce el precio: </label>
                <input type="precio" class="form-control input @error('precio') is-invalid @enderror" id="precio" name="precio" value="{{ old('precio') }}" required>
                @error('precio')
                <span class="invalid-feedback" role="alert">
                    <strong>{{ $message }}</strong>
                </span>
            ...
            <div class="form-group">
                ...
                <input type="stock" class="form-control input @error('stock') is-invalid @enderror" id="stock" name="stock" value="{{ old('stock') }}" required>
                @error('stock')
                <span class="invalid-feedback" role="alert">
                    <strong>{{ $message }}</strong>
                </span>
            ...
            <br>
            <input id="send_button" type="submit" class="btn addButton" value="Añadir producto">
        </form>
    </div>
