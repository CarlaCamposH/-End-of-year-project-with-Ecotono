...
<body>
  ...
  <div class="container-fluid containerB">
    <h1 class="title">TUS FAVORITOS <i class="fas fa-heart heartI" style="color:#97c7c7;"></i> </h1>
    <div class="row">
      @foreach ($data as $item)
      <div class="col-3 producto">
        <a href=" {{ route('details.product',$item->id) }}">
          <img src="{{url('images/productimages/',$item->image1) }}" height="80%" width="100%" />
        </a>
        ...
        <div class="textos">
          ...
          <div class="precio">
            ...
            <form action="{{ route('details.product',$item->id) }}" method="get" style="float:left;">
              @csrf
              <button class="btn DButton">DETALLES</button>
            </form>
            <form action=" {{ route('details.deletefavortio',$item->id) }}" id="{{$item->id_product}}" method="get">
              @csrf
              <button class="btn heartBtn" style="background: transparent;"><i class="fas fa-heart heartI" style="color:red;"></i></button>
            </form>
          </div>
